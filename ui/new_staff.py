...
def date_entry_with_icon(parent, label_text):
    ...
    entry = tk.Entry(inner, width=ENTRY_WIDTH - 3, font=("Segoe UI", 12), relief="flat", bd=0)
    entry.pack(side="left", ipady=6, padx=(6, 0))

    img = Image.open("imgs/icons/calendar.png").resize((18, 18))
    cal_icon = ImageTk.PhotoImage(img)

    btn = tk.Button(inner, image=cal_icon, bg="white", bd=0,
...
def show_staff(content_frame, MAIN_BG):
    ...
    tk.Label(card, text="Add Staff", font=("Segoe UI", 18, "bold"),
             bg=CARD_BG, fg="#2F2F8F").pack(pady=10)
    ...
    def label_dropdown(parent, text, values):
        ...
        tk.OptionMenu(parent, var, *values).pack(anchor="w", fill="x", pady=(2, 14))
        return var
    ...
    city_var = label_dropdown(left, "City *", ["Select City", "Pune", "Mumbai", "Nagpur"])
    ...
    state_var = label_dropdown(right, "State *", ["Select State", "Maharashtra", "Gujarat", "Karnataka"])
    ...
    # ---------- RESET ----------
    def reset_form():
        for e in [fname_entry, lname_entry, email_entry, contact_entry, dob_entry, join_date_entry]:
            e.delete(0, tk.END)
    ...
    tk.Button(btn_frame, text="Save", width=12, bg="#3A2D8F", fg="white",
              command=save_staff).grid(row=0, column=0, padx=10)

    tk.Button(btn_frame, text="Reset", width=12,
              command=reset_form).grid(row=0, column=1, padx=10)

    tk.Button(btn_frame, text="View All Staff", width=18,
              command=open_view_all_staff).grid(row=0, column=2, padx=10)

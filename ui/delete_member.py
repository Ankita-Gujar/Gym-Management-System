...
from db.member_delete import delete_member_by_email


def show_delete(content_frame, MAIN_BG):
    ...
    # ================= CARD =================
    card_border = tk.Frame(content_frame, bg="#B0B3D6", padx=3, pady=3)
    card_border.place(relx=0.5, rely=0.5, anchor="center")

    card = tk.Frame(card_border, bg="#D9DBEE", width=950, height=520)
    card.pack()
    ...
    email_entry.grid(row=0, column=1, padx=10, ipady=4)
    ...
    def auto_load(event=None):
        ...
        result = search_member_by_email(email)
        if not result:
            return
    ...
    def delete_action():
        ...
        if not email:
            messagebox.showwarning("Error", "Enter Email ID")
            return
        ...
        if confirm:
            delete_member_by_email(email)
            tree.delete(*tree.get_children())
            email_var.set("")
            messagebox.showinfo("Deleted", "Member deleted successfully")

    # ================= BIND AUTO LOAD =================
    email_entry.bind("<Return>", auto_load)      # Press Enter
    email_entry.bind("<FocusOut>", auto_load)    # Click outside
